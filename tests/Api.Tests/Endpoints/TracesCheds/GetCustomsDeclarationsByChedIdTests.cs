...
public class GetCustomsDeclarationsByChedIdTests : EndpointTestBase, IClassFixture<WireMockContext>
{
    ...
    [Fact]
    public async Task Get_WhenNotFound_ShouldReturnNotFound()
    {
        var client = CreateClient();
        MockCustomsDeclarationService
            .GetCustomsDeclarationsByFullChedId(ChedId, Arg.Any<CancellationToken>())
            .Returns(Task.FromResult(new List<CustomsDeclarationEntity>()));

        var response = await client.GetAsync(
            TradeImportsDataApi.Testing.Endpoints.TracesCheds.GetCustomsDeclarations(ChedId)
        );
