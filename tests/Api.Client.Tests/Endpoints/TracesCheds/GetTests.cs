...
public class GetTests : WireMockTestBase<WireMockContext>
{
    ...
    [Fact]
    public async Task GetTracesChed_WhenNotFound_ShouldBeNull()
    {
        var result = await Subject.GetTracesChed("unknown", CancellationToken.None);

        result.Should().BeNull();
    }

    [Fact]
    public async Task GetTracesChed_WhenFound_ShouldNotBeNull()
    {
        ...
        WireMock
            .Given(Request.Create().WithPath($"/traces-cheds/{chedId}").UsingGet())
            .RespondWith(
                Response
                    .Create()
                    .WithBody(
                        JsonSerializer.Serialize(
                            new Defra.TradeImportsDataApi.Api.Endpoints.TracesChed.TracesChedResponse(
        ...
        var result = await Subject.GetTracesChed(chedId, CancellationToken.None);
