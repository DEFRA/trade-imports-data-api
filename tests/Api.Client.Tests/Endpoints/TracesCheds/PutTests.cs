...
public class PutTests(WireMockContext context) : WireMockTestBase<WireMockContext>(context)
{
    ...
    [Fact]
    public async Task PutTracesChed_WhenNoEtag_ShouldNotBeNull()
    {
        const string chedId = "CHED";
        var data = new DefraUNVTDCHEDProfile()
        {
        ...
        var act = async () => await Subject.PutTracesChed(chedId, data, etag: null, CancellationToken.None);

        await act.Should().NotThrowAsync();
    }

    [Fact]
    public async Task PutTracesChed_WhenHasEtag_ShouldNotBeNull()
    {
        const string chedId = "CHED";
        var data = new DefraUNVTDCHEDProfile()
        {
        ...
        var act = async () => await Subject.PutTracesChed(chedId, data, etag: "\"etag\"", CancellationToken.None);

        await act.Should().NotThrowAsync();
    }

    [Fact]
    public async Task PutTracesChed_WhenBadRequest_ShouldThrow()
    {
        const string chedId = "CHED";
        var data = new DefraUNVTDCHEDProfile()
        {
        ...
        WireMock
            .Given(
                Request.Create().WithPath($"/traces-cheds/{chedId}").WithBody(JsonSerializer.Serialize(data)).UsingPut()
            )
