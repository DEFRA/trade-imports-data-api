...
public class GetCustomsDeclarationsByChedTests : WireMockTestBase<WireMockContext>
{
    ...
    public async Task GetCustomDeclarationsByChed_WhenFound_ShouldNotBeNull()
    {
        ...
        WireMock
            .Given(Request.Create().WithPath($"/traces-cheds/{chedId}/customs-declarations").UsingGet())
            .RespondWith(
                Response
                    .Create()
                    .WithBody(
        ...
        var result = await Subject.GetCustomsDeclarationsByTracesChedId(chedId, CancellationToken.None);
