...
public class DetailsOfControlledDestinationsTypeTests
{
    [Theory]
    ...
    {
        DetailsOfControlledDestinationsType.IsCommercialTransporter(status).Should().Be(expected);
    }
    ...
    {
        DetailsOfControlledDestinationsType.IsPrivateTransporter(status).Should().Be(expected);
    }
}
